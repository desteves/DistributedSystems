...
public class MasterListener implements Runnable {
	...
	public void run() {
		...
		while (true) {
			try {
				...
				String message; // TODO add timeout?
				// Thread.sleep(500);
	...
	public String RegisterClient(Socket clientSocket, String str) {
		...
		int count = Master.registry.size();

		String name = "CLIENT-" + str;

		String[] arr = str.split("-");

		Master.registry.put(name, new HashMap<String,String>());

		new Thread(new SlaveNodeTracker(clientSocket, name)).start();
	...
	public void DeRegisterClient(String name) {
		...
		HashMap<String, String> removed = Master.registry.remove(name);
