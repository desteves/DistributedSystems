...
def handleArgs(args):
    ...
    u = 100
    try:
        optlist, args = getopt.getopt(args[1:], 'k:i:u:d:o:')
...
if rank == 0: #this is the master process
    ...
    OutputClusters = [None] * (size - 1)

    for i in range(1,size):
        finalop =None
        while 1:
            finalop= comm.recv(source=i)
            if finalop['message_type']=='final_cluster' :
                print 'recieved final cluster'
                break
        OutputClusters[i-1]=finalop
    ...
    #TODO::recieved details should be written to file 
    ...
    xy = {'message_type': "final_cluster", 'xy': cluster_info, 'points' : points , 'clusters' : clusters}
    print 'stopped . Now sending final clusters'
    comm.send(xy,dest=0)
